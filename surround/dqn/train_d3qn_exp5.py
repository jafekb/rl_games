"""Train a D3QN agent - Experiment 5 (match baseline buffer/epsilon hyperparams).

Differences from exp4 (train_d3qn_exp4.py):
  1. Memory capacity: 100K -> 10K (matches baseline DQN; fills faster, less stale data)
  2. Epsilon decay fraction: 0.15 -> 0.01 (matches baseline; greedy by ~ep 1000)
  3. Epsilon end: 0.05 -> 0.01 (matches baseline)
  4. Learning starts: 10K -> 1K env steps (scaled proportionally with buffer)
  N-step=10 and all other hyperparameters kept from exp4.

Log dir: runs/surround/d3qn/exp5/
"""
...
class D3QNTrainer:
    """D3QN trainer exp5: Dueling + Double DQN, uniform replay, n-step returns (n=10)."""
    ...
    def __init__(self) -> None:
        ...
        self.optimizer = torch.optim.AdamW(
            self.policy_net.parameters(), lr=constants.D3QN_EXP5_LR, amsgrad=True
        )

        self.memory = UniformReplayMemory(capacity=constants.D3QN_EXP5_MEMORY_CAPACITY)
        self.n_step_buf = NStepBuffer(n=constants.D3QN_EXP5_N_STEP, gamma=constants.D3QN_GAMMA)

        self.best_steps_survived = 0
        self._total_env_steps = 0
        self._run_metadata = _get_run_metadata()

        if constants.D3QN_EXP5_LOG_DIR.exists():
            raise FileExistsError(
                f"Log dir already exists: {constants.D3QN_EXP5_LOG_DIR}."
                " Remove it before a fresh run."
            )
        print(f"Saving checkpoint and run information to {constants.D3QN_EXP5_LOG_DIR}")
        logging.getLogger("tensorboardX").setLevel(logging.ERROR)
        self.writer = SummaryWriter(log_dir=str(constants.D3QN_EXP5_LOG_DIR))
    ...
    def _optimize_model(self) -> dict[str, float] | None:
        if len(self.memory) < constants.D3QN_EXP5_LEARNING_STARTS:
            return None
        ...
        gamma_n = constants.D3QN_GAMMA**constants.D3QN_EXP5_N_STEP
    ...
    def _save_checkpoint(self, episode_index: int, steps_survived: int | None = None) -> None:
        ...
        save_checkpoint(
            constants.D3QN_EXP5_POLICY_NET_LATEST,
        ...
        constants.D3QN_EXP5_CHECKPOINT_METADATA.write_text(
            json.dumps(json_meta, indent=2), encoding="utf-8"
        )
        if ep % constants.D3QN_EXP5_CHECKPOINT_INTERVAL == 0:
            path = constants.D3QN_EXP5_CHECKPOINT_DIR / f"policy_net_{ep:04d}.pt"
            save_checkpoint(
    ...
    def run(self) -> None:
        for episode_index in trange(constants.D3QN_NUM_EPISODES):
            self._current_epsilon = epsilon_for_episode(
                ...
                constants.D3QN_EXP5_EPS_DECAY_FRACTION,
                constants.D3QN_EPS_START,
                constants.D3QN_EXP5_EPS_END,
            )
            ...
            for t in trange(constants.MAX_CYCLES, leave=False):
                ...
                if self._total_env_steps % constants.D3QN_EXP5_UPDATE_EVERY == 0:
                ...
                if done:
                    ...
                    if steps_survived > self.best_steps_survived:
                        ...
                        save_checkpoint(
                            constants.D3QN_EXP5_POLICY_NET_BEST,
                            self.policy_net.state_dict(),
                            steps_survived=steps_survived,
                            **{
                                **self._run_metadata,
